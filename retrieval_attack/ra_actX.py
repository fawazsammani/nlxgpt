...
def sample_sequences(img, model, input_ids, segment_ids, tokenizer):
    ...
    SPECIAL_TOKENS = ['<|endoftext|>', '<pad>', '<answer>', '<explanation>']
...
img_size = 224
ckpt_path = 'ckpts/'
nle_data_test_path = 'nle_data/ACT-X/actX_test.json'
img_folder = 'images/mpi/'
max_seq_len = 40
load_from_epoch = 5
no_sample = True   
top_k =  0
...
i2i = json.load(open('ra/retrieval_attack_data_i2i_actX.json', 'r'))
...
# same input ids since there is no different quetion/hypothesis
input_ids, segment_ids = get_inputs(tokenizer)

for i,(img_name, retrieved_images) in enumerate(i2i.items()):
    
    all_vectors = []
    p_retrieved_images = retrieved_images[:15]
    
    for ri in p_retrieved_images:
        img_path = img_folder + ri
    ...
    print("\rFinished {}/{} images".format(i+1, len(i2i)), end='          ')
...
print("RA Image2Image @5: {}".format(mean5_dist ))
print("RA Image2Image @10: {}".format(mean10_dist))
print("RA Image2Image @15: {}".format(mean15_dist))